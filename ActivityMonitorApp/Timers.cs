...
namespace ActivityMonitorApp
{
    internal class Timers
    {
        ...
        /// Die Methode started automatisches Speichern
        /// </summary>
        public void SwitchToFullSave()
        {
            if (Timer != null)
            {
                if (!Started)
                {
                    Timer.Elapsed += SwitchToFullSave;
                    Timer.Enabled = true;
                    Timer.AutoReset = true;
        ...
        private void SwitchToFullSave(object source, ElapsedEventArgs e)
        {
            Console.WriteLine("");
            Console.WriteLine("Du bist jetzt aktiv");
